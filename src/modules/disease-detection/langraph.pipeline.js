...
async function processDiseaseDetection(state) {
  ...
  try {
    ...
    if (!analysis || !analysis.disease) {
      throw new Error(
        "No disease detection results available from image analysis"
      );
    }
      ...
      await Analysis.findByIdAndUpdate(state.analysisId, {
        ...
        $push: {
          processingSteps: {
            ...
            status: "completed",
            result: {
              message: "Invalid image - no recommendations generated",
              isInvalidImage: true,
              timestamp: new Date(),
            },
          },
...
function enhanceTreatments(treatments, location, cropType) {
  ...
  return treatments.map((treatment) => {
    ...
    return {
      ...
      description:
        treatment.description || treatment.method || "Apply as recommended",
  ...
  try {
    ...
    const finalResult = {
      ...
      metadata: {
        ...
        isInvalidImage: state.diseaseDetection.isInvalidImage || false,
      },
    };
    ...
    await Analysis.findByIdAndUpdate(state.analysisId, {
      ...
      $push: {
        processingSteps: {
          ...
          status: "completed",
          result: {
            message: state.diseaseDetection.isInvalidImage
              ? "Invalid image processing completed"
...
export function createAnalysisPipeline() {
  ...
  const edges = [
    ...
    [
      "generateRecommendations",
      { finalizeResults: "finalizeResults", handleError: "handleError" },
    ],
    ["finalizeResults", { [END]: END, handleError: "handleError" }],
  ];

  edges.forEach(([node, mapping]) =>
...
export async function executeAnalysisPipeline(analysisData) {
  ...
  try {
    console.log(
      "Starting pipeline execution with state:",
    ...
    if (
      ...
      result.currentStep === "error_handled"
    ) {
      const errorMessage =
